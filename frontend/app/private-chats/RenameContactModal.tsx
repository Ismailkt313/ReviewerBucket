      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border/80 bg-neutral-50/50 dark:bg-neutral-900/30">
          <div className="flex flex-col">
            <h2 id="rename-dialog-title" className="text-sm font-bold text-foreground">
              Rename contact
            </h2>
            <p id="rename-dialog-desc" className="text-xs text-muted mt-0.5">
          ...
          <button
            ...
            className="p-1.5 rounded-lg text-secondary hover:text-foreground hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors disabled:opacity-40"
          ...
          >
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label htmlFor="contact-nickname-input" className="text-xs font-medium text-secondary">
                  Private Nickname
                </label>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 rounded-xl border border-border bg-background text-sm text-foreground placeholder:text-muted/60 focus:outline-none focus:ring-2 focus:ring-focus focus:border-transparent transition-all disabled:opacity-50"
              />
            </div>

            {error && (
              <p className="text-xs text-red-600 dark:text-red-400 font-medium px-0.5" role="alert">
                {error}
              </p>
            )}
            ...
            <div className="pt-2 flex items-center justify-between gap-2">
              <div>
                {isCustomName && (
                  <button
                    ...
                    onClick={handleReset}
                    disabled={isSubmitting || isResetting}
                    className="inline-flex items-center gap-1.5 text-xs text-muted hover:text-foreground transition-colors disabled:opacity-40 py-2 px-1 focus-visible:outline-none focus-visible:underline"
                  >
                    {isResetting ? (
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  onClick={onClose}
                  disabled={isSubmitting || isResetting}
                  className="px-3.5 py-2 rounded-xl text-xs font-medium text-secondary hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors disabled:opacity-40"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl bg-accent text-background text-xs font-semibold hover:opacity-90 transition-all disabled:opacity-40 shadow-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus"
                >
                  {isSubmitting ? (
                    <>
                      <Loader2 className="w-3.5 h-3.5 animate-spin" />
